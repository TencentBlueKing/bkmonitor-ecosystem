    ...
    def _loop_query_hello_world(self):
        ...
        while not self.stopped.wait(INTERVAL):
            try:
                ...
                logger.error("[query_hello_world] got error -> %s", e)

        logger.info("[%s] loop_query_hello_world stopped", self)

    def _query_hello_world(self):
        with self.tracer.start_active_span("caller/query_hello_world") as _:
            logger.info("[query_hello_world] send request")
            response = requests.get(self.config.url)
            logger.info("[query_hello_world] received: %s", response.text)
