...
自定义日志 SDK 上报使用 OTLP/HTTP 协议 进行数据传输。SDK 内部封装了协议细节，开发者只需配置 SDK 参数，SDK 会自动按标准格式将日志上报到蓝鲸日志平台。
...
| `service_name` | 是 | 服务名称，用于标识日志来源，会自动写入 Resource.attributes["service.name"]。 |
| `environment` | 否 | 环境标识（如 production、staging、dev），会自动写入 Resource.attributes["deployment.environment"]。 |
| `log_level` | 否 | 日志采集级别（如 INFO、WARN、ERROR），低于该级别的日志将被过滤。 |
| `max_batch_size` | 否 | 单次上报的最大日志条数，默认值由 SDK 决定，通常为 100 条。 |
| `flush_interval` | 否 | 日志上报的间隔时间（秒），SDK 会按此间隔批量上报，默认值由 SDK 决定。 |
...
SDK 上报的数据结构遵循 OTLP 标准协议，开发者无需手动构造，SDK 会自动将日志转换为以下格式：

```json
ExportLogsServiceRequest
  └── resourceLogs[]                  // 按资源分组的日志集合
        ├── resource                  // 产生日志的实体信息
        │     └── attributes[]        // 资源属性（服务名、环境等）
        ├── scopeLogs[]               // 按 instrumentation scope 分组的日志
        │     ├── scope               // 产生日志的库/模块/logger 信息
        │     └── logRecords[]        // 实际日志记录列表
        │           ├── timeUnixNano           // 日志发生时间（Unix 纳秒时间戳）
        │           ├── observedTimeUnixNano   // 日志被观测到的时间
        │           ├── severityNumber         // 标准化日志级别数值
        │           ├── severityText           // 日志级别文本（INFO、WARN、ERROR）
        │           ├── body                   // 日志正文（字符串或结构化对象）
        │           ├── attributes[]           // 日志事件的附加属性
        │           ├── traceId                // 关联的 Trace ID（十六进制字符串）
        │           └── spanId                 // 关联的 Span ID（十六进制字符串）
        └── schemaUrl                  // schema 地址
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `resourceLogs` | `array<ResourceLog>` | 是 | 按资源分组的日志集合，SDK 自动按服务名等维度分组。 |

`ResourceLog`：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `resource` | `Resource` | 否 | 产生日志的实体，例如服务、容器、Pod 或进程。SDK 自动填充服务名、环境等信息。 |
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `attributes` | `array<KeyValue>` | 否 | 资源属性，SDK 自动填充 service.name、deployment.environment 等。 |
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `scope` | `InstrumentationScope` | 否 | 产生日志的库、模块或 logger 信息。SDK 自动填充 logger 名称和版本。 |
| `logRecords` | `array<LogRecord>` | 是 | 实际日志记录列表，SDK 按配置批量填充。 |
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `name` | `string` | 否 | 产生日志的库、模块或 logger 名称，SDK 自动填充（如 myapp-logger）。 |
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `timeUnixNano` | `string` | 否 | 日志发生时间，Unix 纳秒时间戳。OTLP JSON 中 `fixed64` 使用字符串表达。SDK 自动填充。 |
| `observedTimeUnixNano` | `string` | 否 | 日志被采集系统观测到的时间。SDK 自动填充。 |
| `severityNumber` | `integer` | 否 | 标准化日志级别数值，SDK 根据日志级别自动映射。 |
| `severityText` | `string` | 否 | 原始日志级别文本，例如 `INFO`、`WARN`、`ERROR`。SDK 自动填充。 |
| `body` | `AnyValue` | 否 | 日志正文，可以是字符串或结构化对象。SDK 自动填充业务日志内容。 |
| `attributes` | `array<KeyValue>` | 否 | 当前日志事件的附加属性。SDK 可配置自定义维度。 |
| `droppedAttributesCount` | `integer` | 否 | 被丢弃的日志属性数量。 |
| `flags` | `integer` | 否 | Trace flags。低 `8` 位对应 W3C Trace Context flags。 |
| `traceId` | `string` | 否 | Trace ID，OTLP JSON 中使用十六进制字符串。SDK 自动关联 Trace 上下文。 |
| `spanId` | `string` | 否 | Span ID，OTLP JSON 中使用十六进制字符串。SDK 自动关联 Span 上下文。 |
  ...
  "resourceLogs": [
    {
      ...
      "scopeLogs": [
        {
          "scope": {
            "name": "com.example.order.controller",
            "version": "1.0.0"
          },
          "logRecords": [
            {
              "timeUnixNano": "1712800000000000000",
              "observedTimeUnixNano": "1712800000001000000",
              "severityNumber": 9,
              "severityText": "INFO",
              "body": {
                "stringValue": "Order created successfully, orderId=12345"
              },
              "attributes": [
                {
                  "key": "order_id",
...
以下是使用 curl 模拟 SDK 上报的完整请求示例，可用于验证上报链路是否畅通：
          ...
          "logRecords": [
            {
              ...
              "severityNumber": 9,
              "severityText": "INFO",
              "body": { "stringValue": "custom log sdk report demo" },
              "attributes": [
...
`severityNumber` 是 OpenTelemetry 对日志级别的标准化数值。数值越大，日志越严重。SDK 会自动将业务日志级别映射为标准的 `severityNumber` 值：
...
| 日志级别 | severityText | severityNumber |
| --- | --- | --- |
| TRACE | TRACE | 1 |
| DEBUG | DEBUG | 5 |
| INFO | INFO | 9 |
...
* 了解 <a href="{{docs.logs.learn.sdk_python}}" target="_blank">Python-日志（SDK）上报</a>。

* 了解 <a href="{{docs.logs.learn.sdk_c}}" target="_blank">C++-日志（SDK）上报</a>。

* 了解 <a href="{{docs.logs.learn.sdk_java}}" target="_blank">Java-日志（SDK）上报</a>。

* 了解 <a href="{{docs.logs.learn.sdk_go}}" target="_blank">Go-日志（SDK）上报</a>。
