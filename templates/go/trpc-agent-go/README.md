...
本文和配套样例只演示 **OTLP/HTTP** 上报。需要使用 OTLP/gRPC 时，请参考<a href="{{ECOSYSTEM_CODE_ROOT_URL}}/docs/open/go/otlp/README.md" target="_blank">Go（OpenTelemetry SDK）接入</a>，本文不再重复提供 gRPC 初始化代码。
...
完整初始化和关闭逻辑请参考 <a href="{{ECOSYSTEM_CODE_ROOT_URL}}/examples/go-examples/trpc-agent-go/telemetry.go" target="_blank">telemetry.go</a>。该样例还通过 `otlploghttp` 和 `otelslog` 上报应用日志。
...
* <a href="{{ECOSYSTEM_CODE_ROOT_URL}}/docs/open/go/otlp/README.md" target="_blank">Go（OpenTelemetry SDK）接入：gRPC 和其他 OTLP 配置</a>
