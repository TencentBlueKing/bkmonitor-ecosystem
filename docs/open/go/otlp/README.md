...
接入 Web 框架、HTTP 客户端、数据库或消息队列等，先在 <a href="https://github.com/open-telemetry/opentelemetry-go-contrib" target="_blank">OpenTelemetry Go Contrib</a> 和组件自身文档中查找 instrumentation。
...
-e ENABLE_LOGS="true" helloworld-go:latest
```
...
| `OTLP_ENDPOINT`      | `"127.0.0.1:4318"` | 【必须】OT 数据上报地址，支持以下协议：<br />  `gRPC`：`http://127.0.0.1:4317`<br /> `HTTP`：`127.0.0.1:4318`（demo 使用该协议演示上报） |
| `PROFILING_ENDPOINT` | `"http://127.0.0.1:4318/pyroscope"`                | 【可选】Profiling 数据上报地址。                                                                                                                                    |
| `ENABLE_TRACES`      | `true`                  | 是否启用调用链上报。                                                                                                                                               |
| `ENABLE_METRICS`     | `true`                 | 是否启用指标上报。                                                                                                                                                |
| `ENABLE_LOGS`        | `true`                    | 是否启用日志上报。                                                                                                                                                |
| `ENABLE_PROFILING`   | `http://127.0.0.1:4318/pyroscope`                  | 是否启用性能分析上报。                                                                                                                                              |
