...
本示例使用 `opentelemetry-java-http-client` 处理出站请求。相关依赖见 <a href="https://github.com/TencentBlueKing/bkmonitor-ecosystem/tree/master/examples/java-examples/helloworld/build.gradle" target="_blank">build.gradle</a>：
...
在 Java 中，通过 `setSpanKind` 指定 Kind。例如，表示一次向外部服务发起的请求：

```java
import io.opentelemetry.api.trace.SpanKind;

Span span = GlobalOpenTelemetry.getTracer("helloworld").spanBuilder("ClientDemo/request").setSpanKind(SpanKind.CLIENT).startSpan();
span.end();
```
