...
接入 Web 框架、HTTP 客户端、数据库或消息队列等，先在 <a href="https://github.com/open-telemetry/opentelemetry-rust-contrib" target="_blank">OpenTelemetry Rust Contrib</a> 和组件自身文档中查找 instrumentation。

已有成熟组件时，优先使用组件完成协议层 Span、语义属性、指标和上下文传播；手动 Span 留给业务操作。

#### 2.4.1 选择埋点组件

OpenTelemetry Rust 生态中常用的 instrumentation：
...
| `OTLP_ENDPOINT`      | `"http://127.0.0.1:4318"`                | 【必须】OT 数据上报地址，请根据页面指引提供的接入地址进行填写。Rust demo 使用 `HTTP/protobuf` 协议，并为该地址追加对应信号路径。 |
