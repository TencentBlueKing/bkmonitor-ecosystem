...
| `OTLP_ENDPOINT` | 是 | ❗❗【非常重要】OTLP HTTP 根地址。国内站点默认是「http://127.0.0.1:4318」，其他环境、跨云场景请根据页面接入指引填写；上报调用链时在末尾追加 `/v1/traces`。 |
...
# ❗❗【非常重要】OTLP HTTP 根地址。国内站点默认是「http://127.0.0.1:4318」，其他环境、跨云场景请根据页面接入指引填写。
